...
<head>
    ...
    <title>@lang("messages.crearUsuario")</title>
    <!--Icono de la web-->
    <link rel="icon" href="{{asset('imgs/icono.ico')}}" type="image/x-icon">
    <!-- Link a CSS y JS propios -->
    <link rel="stylesheet" href="{{ asset('css/usuarios/signup.css') }}">
    <!-- Recoge en una variable si el email es duplicado o no -->
    <script type="text/javascript">
        var emailDuplicado = "<?php echo isset($emailDuplicado) ? $emailDuplicado : null; ?>";
    </script>
    <script type="text/javascript" src="{{ asset('js/usuarios/signup.js') }}"></script>
</head>
    ...
    <div class="container">
        <div class="form-container">
            <div class="section-header text-center w-100">
                <h1 class="primary-heading my-4">@lang("messages.crearNuevoUsuario")<span class="fullstop">.</span></h1>
            </div>
            <form method="POST" action="{{ route('usuarios.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="form-input">
                    <div class="name-input">
                        <input type="text" name="name" id="name" placeholder="@lang('messages.username')" required />
                        <input type="number" name="telefono" id="telefono" placeholder="@lang('messages.telefono')" />
                    </div>
                    <input type="email" name="email" id="email" placeholder="@lang('messages.email')" required />
                    <input type="password" name="pass" id="pass" placeholder="@lang('messages.password')" required />
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <select class="form-select" name="rol" id="rol">
                            ...
                            <option value="{{lcfirst($rol->name)}}">{{$rol->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="btn-input mt-3">
                        <button id="btnVolver" type="button" class="secondary-btn">
                            <i class="fa-solid fa-circle-chevron-left mx-2"></i>@lang("messages.volver")
                        </button>
                        <button type="submit" class="primary-btn">
                            <i class="fa-solid fa-square-plus mx-2"></i>@lang("messages.crearCuenta")
                        </button>
                    </div>
                </div>
            </form>
