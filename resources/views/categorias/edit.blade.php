...
<head>
    ...
    <title>@lang('messages.editarCategoria')</title>
    <!--Icono de la web-->
...
<body>
        ...
        <div class="d-flex align-items-center justify-content-center">
            <div class="row border border-secondary-subtle rounded">
                ...
                <div class=" py-2 px-4 col d-flex flex-column align-content-center">
                    ...
                    <h3>@lang('messages.editCategoria')</h3>
                    <p>
                        {{__('messages.editCategoriaText')['parrafo1']}}<br />
                        {{__('messages.editCategoriaText')['parrafo2']}}
                    </p>
    ...
    <div class="main-connection-div m-3">
        <div class="p-0">
            <form method="post" id="edit-form" action="{{ route('categorias.update', $categoria->id) }}">
                @csrf
                @method('PUT')
                <div class="d-grid" id="campos">
                    ...
                    <div class="input-field campo">
                        ...
                        <input id="nom{{$language}}" name="nom{{$language}}" type="text" class="form-control" value="{{Lang::get('categorias.'.$categoria->tipo, [], $language)}}" placeholder="@lang('messages.placeholder_categoria')" required>
                    </div>
                    @endforeach
                    ...
                    <div class="input-field border border-dark border-opacity-25 rounded" id="empresas">
                        ...
                        <div class="list-group" id="catContainer">
                            <button type="button" class="list-group-item list-group-item-action {{($categoria->id_empresa == '')?'active':''}}" value="0" onclick="toggle(this)">--@lang("messages.ninguna")--</button>
                            @foreach($empresas as $empresa)
                            <button type="button" class="list-group-item list-group-item-action {{($categoria->id_empresa == $empresa->id)?'active':''}}" value="{{$empresa->id}}" onclick="toggle(this)">{{$empresa->nombre}}</button>
                            @endforeach
